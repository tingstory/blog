...
- 在手机或电脑上打开一些任意一个链接
  - [推荐下载链接](https://d.orxs.me/aff/JT8F)
  - [备用下载链接](https://d.antss049.xyz/aff/JUFT)
- 下载，安装，打开软件或 App
- 点击 `线路地址`，对 `蚂蚁出国专线`（初始只有 300 MB 流量）和 `公共免费线路`（无限流量，不过速度也不赖）进行测速，选择一个最快的线路，点击左上角返回主界面
- 在主界面点击右上角的小齿轮 ⚙️，选择 `路由`
  - 首选：`绕过局域网和中国大陆地址` 或 `GFW 列表`
  - 如果分流模式（上面那两个模式）下无法访问个别网站，临时切换到 `全局模式`
...
再再如果，就买实体手机 SIM 卡（电脑不适用），例如 [Google Fi](https://fi.google.com/about/) 或者中国联通的 [CUniq](https://www.cuniq.com/uk)。
